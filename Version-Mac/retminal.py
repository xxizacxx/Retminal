...
# --- Plateforme : cette version marche sur Mac ET Windows ---
IS_WIN = sys.platform == "win32"
IS_MAC = sys.platform == "darwin"
# Le drapeau "cache la fenetre console" n'existe que sur Windows.
NO_WINDOW = subprocess.CREATE_NO_WINDOW if IS_WIN else 0
NEW_GROUP = subprocess.CREATE_NEW_PROCESS_GROUP if IS_WIN else 0
...
_APP_DIR = None
ENV_NAMES = ("secret.env", ".env")


def _bundle_dir():
    # ou sont les fichiers livres dans l'appli (lecture seule)
    if getattr(sys, "frozen", False):
...
def _app_dir():
    # dossier de travail (lecture ET ecriture des configs/.env)
    global _APP_DIR
    if _APP_DIR:
        return _APP_DIR
    if getattr(sys, "frozen", False) and IS_MAC:
        d = os.path.expanduser("~/Library/Application Support/Retminal")
        try:
            os.makedirs(d, exist_ok=True)
            import shutil
            for fn in ("servers.json", "customcommands.json", "secret.env", ".env", "settings.json"):
                dst = os.path.join(d, fn)
                src = os.path.join(_bundle_dir(), fn)
...
def resolve_path(fname):
    ...
    candidates = list(ENV_NAMES) if fname in ENV_NAMES else [fname]
    for base in (_app_dir(), os.getcwd()):
...
def read_token(ftype, fname, selector, password=None):
    ...
    if ftype == "ENV":
        if not selector:
            raise ValueError("ENV// a besoin d'une cle, ex: ENV//secret.env!VPS_HOST")
...
class Retminal:
    ...
    def _build_ui(self):
        ...
        if IS_MAC:
            ...
            for _seq, _fn in (
                ("<Command-c>", self._on_ctrl_c),
                ("<Command-d>", self._on_ctrl_d),
                ("<Command-Return>", self._editor_key_nextline),
                ("<Command-s>", self._editor_key_save),
                ("<Command-S>", self._editor_key_save),
                ("<Command-k>", self._editor_key_delline),
                ("<Command-K>", self._editor_key_delline),
                ("<Command-v>", self._on_paste),
                ("<Command-V>", self._on_paste),
                ("<Command-ugrave>", self._cycle_shell),
            ):
                self.input_entry.bind(_seq, _fn)
            self.root.bind("<Command-t>", lambda e: self._new_tab())
    ...
    def _emoji_image(self, s):
        ...
        try:
            ...
            if IS_MAC:
                raise RuntimeError("emoji natif Tkinter sur Mac")
    ...
    def _prompt_segments(self):
        ...
        sh = self._cur_shell()
        if not self.claude_mode and sh["kind"] == "unix":
            return [
                (self._wsl_user, "bright"),
                ("@", "dim"),
                ("mac" if IS_MAC else "unix", "cyan"),
                (":", "dim"),
    ...
    def _path_list_now(self, context, distro, abs_dir):
        ...
        if context == "wsl":
            r = subprocess.run(
                ...
                creationflags=NO_WINDOW, timeout=6,
            )
            res = []
    ...
    def _scan_local_commands(self):
        ...
        if sh["kind"] == "wsl":
            ...
            found = self._scan_unix_commands()
        else:
            found = self._scan_windows_commands()
        cache[key] = found
        self._local_cmds = found

    def _scan_unix_commands(self):
        found = []
        seen = set()
    ...
    def _maybe_cd_unix(self, cmd):
        ...
        if len(parts) == 1:
            ...
            if arg in ("~", ""):
                ...
                target = os.path.join(self.wsl_cwd, arg)
        target = os.path.normpath(target)
        if os.path.isdir(target):
            self.wsl_cwd = target
        else:
            self._insert("cd: dossier introuvable : " + target + "\n", "err")
        self._write_prompt()
        return True
    ...
    def _maybe_cd(self, cmd):
        ...
        if self._shell_kind() == "unix":
            return self._maybe_cd_unix(cmd)
        s = cmd.strip()
    ...
    def _init_shells(self):
        ...
        if not IS_WIN:
            import shutil
            self._wsl_user = os.environ.get("USER", "user")
            self._wsl_home_path = os.path.expanduser("~")
            self.wsl_cwd = self._wsl_home_path
            ordered = []
            seen = set()
            usershell = os.environ.get("SHELL", "")
            if usershell and os.path.exists(usershell):
                nm = os.path.basename(usershell)
                ordered.append((nm, usershell))
                seen.add(nm)
            for nm in ("zsh", "bash", "sh"):
                if nm in seen:
                    continue
                p = shutil.which(nm) or ("/bin/" + nm if os.path.exists("/bin/" + nm) else None)
                if p:
                    ordered.append((nm, p))
                    seen.add(nm)
            if not ordered:
                ordered = [("sh", "/bin/sh")]
            for nm, path in ordered:
                self.shells.append({"key": nm, "name": nm, "kind": "unix", "exe": path})
            self.shell_index = 0
            return
        distros = []
        try:
    ...
    def _scan_wsl_commands(self, distro):
        ...
        try:
            o = subprocess.run(
                ...
                capture_output=True, stdin=subprocess.DEVNULL,
                creationflags=NO_WINDOW, timeout=12,
            ).stdout
            seen = set()
    ...
    def _run_shell(self, cmd, shell=None):
        ...
        kind = sh["kind"]
        if kind == "unix":
            argv = [sh.get("exe", "/bin/zsh"), "-lc", cmd]
            popen_kwargs = dict(
                cwd=self.wsl_cwd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
        ...
        try:
            if IS_WIN:
                subprocess.run(
                    ...
                    creationflags=NO_WINDOW, capture_output=True, timeout=5,
                )
            else:
                import signal
                try:
    ...
    def _friendly_claude_error(self, txt):
        ...
        if self.connected:
            return (
                ...
                "      puis colle le code dans secret.env :  CLAUDE_CODE_OAUTH_TOKEN=...\n"
    ...
    def cmd_sysinfo(self, cmd):
        if not IS_WIN:
            self._insert("  ℹ️  sysinfo (moniteur du Mac) pas encore dispo sur la version Mac.\n", "dim")
            self._insert("  Le reste marche : terminal, editeur, config, SSH/VPS, moniteur serveur...\n", "dim")
            self._write_prompt()
            return
        self._sysmon_start()
    ...
    def cmd_run(self, cmd):
        ...
        try:
            ...
            self._insert("Lancement de '" + app + "' sur ton ordi.\n", "bright")
        except Exception as e:
    ...
    def _cfg_update_worker(self):
        ...
        try:
            ...
            g = subprocess.run(["git", "-C", d, "pull"], capture_output=True,
                               stdin=subprocess.DEVNULL, creationflags=NO_WINDOW, timeout=120)
            ...
            if os.path.isfile(build):
                ...
                b = subprocess.run([build], cwd=d, capture_output=True, stdin=subprocess.DEVNULL,
                                   creationflags=NO_WINDOW, timeout=600, shell=True)
    ...
    def _cfg_verify_worker(self):
        ...
        has_env = any(os.path.isfile(os.path.join(d, n)) for n in ENV_NAMES)
        lines.append("  ✅ secret.env present" if has_env else "  ⚠ secret.env absent (IP/mdp du VPS)")
    ...
    def cmd_connect(self, cmd):
        ...
        if not host or not user or not password:
            self._insert("[!] Aucun serveur dans servers.json et secret.env incomplet.\n", "err")
            self._write_prompt()
            return
